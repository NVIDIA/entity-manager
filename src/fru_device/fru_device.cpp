...
// Downstream: PCIe slot mapping support
using VariantType = std::variant<uint32_t, std::string>;
std::unordered_map<uint32_t, VariantType> i2cPcieMappings;
// Downstream: per-(bus,address) blacklist (uses {bus,address} object format)
               ...
               sdbusplus::asio::object_server& objServer)
{
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            {
                lg2::debug("device at bus {BUS} address {ADDR} busy", "BUS",
                           bus, "ADDR", ii);
                continue;
            }
            // probe
...
std::set<size_t> loadBlocklist(const char* path)
{
    ...
    // Downstream: parse addresses as {bus, address} objects into per-(bus,addr)
    // blacklist
    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    if (i2cPcieMappings.contains(bus))
    {
        ...
        prunedProductName =
            std::regex_replace(prunedProductName, suffixPattern, "_");
        ...
        {
            prunedProductName =
                prunedProductName.substr(0, findLastUndercore + 1);
        }
                 ...
                 busIface != dbusInterfaceMap.end();)
            {
                ...
                if (busIface->first.first == static_cast<size_t>(busNum))
                {
                    ...
                    if (busIface->first.first == static_cast<size_t>(rootBus))
                    {
                        ...
                        {
                            if (!deviceMap->second->contains(
                                    busIface->first.second))
                            {
                                objServer.remove_interface(busIface->second);
