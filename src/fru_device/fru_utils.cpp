    ...
    std::flat_map<std::string, std::string, std::less<>>& result)
{
         ...
         area <= fruAreas::fruAreaProduct; ++area)
    {
        ...
        {
            lg2::error("FRU area extends past the end of FRU data");
            return resCodes::resErr;
        }
    ...
    size_t fruAreaEndOfFieldsOffset, size_t fruAreaEndOffset)
{
    ...
    std::span<const uint8_t> fruDataView{fruData};

    fruData[checksumLoc] = calculateChecksum(fruDataView.begin() + fruAreaStart,
                                             fruDataView.begin() + checksumLoc);
    return checksumLoc;
}
