...
resCodes formatNvmeVid(const std::vector<uint8_t>& nvmeBytes,
                       std::flat_map<std::string, std::string>& result)
{
...
{
    std::flat_map<std::string, std::string> formattedNvme;
    ...
    int bus, int file, const std::string& errorHelp)
{
    ...
    for (retry = 0; retry < 3; retry++)
    {
        ...
        {
            std::vector<uint8_t> device;
            device.reserve(nvme::vendorIdSize);
            std::copy_n(blockData.begin(), nvme::vendorIdSize,
                        std::back_inserter(device));
