...
// std::vector<bool> is specialized in C++ to use bit-packing, making its
// element reference type std::_Bit_reference, which sdbusplus cannot serialize.
// Represent boolean arrays as std::vector<uint8_t> on D-Bus to work around
// this.
template <>
inline void addArrayToDbus<bool>(
    const std::string& name, const nlohmann::json& array,
    sdbusplus::asio::dbus_interface* iface,
    sdbusplus::asio::PropertyPermission permission,
    nlohmann::json& systemConfiguration, const std::string& jsonPointerString)
{
    std::vector<uint8_t> values;
    for (const auto& property : array)
    {
        const auto* ptr = property.get_ptr<const nlohmann::json::boolean_t*>();
        if (ptr != nullptr)
        {
            values.emplace_back(*ptr ? 1 : 0);
        }
    }
    ...
    else
    {
        iface->register_property(
            ...
            [&systemConfiguration,
             jsonPointerString{std::string(jsonPointerString)}](
                const std::vector<uint8_t>& newVal, std::vector<uint8_t>& val) {
                val = newVal;
                if (!setJsonFromPointer(jsonPointerString, val,
                                        systemConfiguration))
                {
